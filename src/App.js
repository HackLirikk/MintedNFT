...
const OPENSEA_LINK = 'https://testnets.opensea.io/collection/monkey-42';
const TOTAL_MINT_COUNT = 50;

const CONTRACT_ADDRESS = "0xFf44D48515cD41F633DcAC9F22a665efbC4631f3";
...
const App = () => {
  ...
  const setupEventListener = async () => {
    try {
      ...
      if (ethereum) {
        ...
        connectedContract.on("Monkey", (from, tokenId) => {
  ...
  const askContractToMintNft = async () => {
      try {
        ...
        if (ethereum) {
          ...
          let nftTxn = await connectedContract.safeMint(account, {
          ...
          await nftTxn.wait()
          console.log(nftTxn)
          console.log(`Mined, tee transaction: https://testnet.bscscan.com/tx/${nftTxn.hash}`)
  ...
  const renderNetworkPrompt = () => (
    alert("Hello there, This app is built on the bnb testnet and it looks like you are on a different ethereum network. Please switch to the Bscscan testnet to continue")
  )
    ...
    <div className="App">
      ...
      <div className="container">
        <div className="header-container">
          <p className="header gradient-text">Monkey NFT Collection</p>
          <a
